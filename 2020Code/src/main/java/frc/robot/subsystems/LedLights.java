...
public class LedLights extends Subsystem {
    ...
    private final Map<String, String> lastCommand = new HashMap<String, String>() {
  ...
  /**
   * In case we get complaints of bling subsystem being distracting We will have
   * the ability to turn off flashy stuff
   */
  ...
  /*
   * Used to tell the bling command whether or not to display green or nothing
   * during autonomous
   */
  ...
  private final int pixels = 120;

  /**
   * Will be true if the battery level is critical, in which case it will override
   * all other signals to display the critical battery warning
   */
  ...
  /**
   * Tells other subsystems if the bling system has displayed a battery critical
   * sigh yet.
   * 
   ...
   */
  public void toggleAutoState(final boolean good) {
    if (good) {
  ...
  /**
   * Returns the value of autostate to tell bling command whether or not to
   * display a green during autonomous.
   * 
   * @return True if autonomous is good, false otherwise.
   */
  public String getSpecialState() {
    final String returnable = specialState;
    specialState = "";
    return returnable;
  }
  ...
  /**
   * Returns the value of the distance shower variable. Used to determine whether
   * or not the drivers want to see distance when lining up.
   * 
  ...
  /**
   * This command will toggle if we're displaying flashy patterns in case of
   * possible complaints, a button on the driver joystick will call this to toggle
   * it.
   */
    ...
    if (!flashyOff) {
      ...
      // If the command was lower than 9, show a command 6.
        ...
        command = ("F6C" + lastCommand.get("colour") + "D" + lastCommand.get("delay") + "B"
            + lastCommand.get("brightness") + "P" + lastCommand.get("startPoint") + "Q" + lastCommand.get("endPoint")
            + "R500E6Z");
      ...
      if (lastCommand.get("pattern") == "0")
        ...
        command = ("F" + lastCommand.get("pattern") + "C" + lastCommand.get("colour") + "D" + lastCommand.get("delay")
            + "B" + lastCommand.get("brightness") + "P" + lastCommand.get("startPoint") + "Q"
            + lastCommand.get("endPoint") + "R500E" + lastCommand.get("pattern") + "Z");

      else
        command = ("F" + lastCommand.get("pattern") + "C" + lastCommand.get("colour") + "D" + lastCommand.get("delay")
            + "B" + lastCommand.get("brightness") + "R500E" + lastCommand.get("pattern") + "Z");
  ...
  /**
   * This function should be run at the beginning of autonomous to get the proper
   * light pattern.
   */
  public void auto() {
  ...
  /**
   * Just displays a fun rainbow of colours during teleop when we're not doing
   * anything.
   */
  ...
  /**
   * This function will run whenever we want to display the battery voltage
   * output. Will run automatically at startup.
   * 
   ...
   */
  public void batteryInd(final double percent, final boolean criticalStatus) {
   ...
   */
  public void showDistance(final boolean pegIn) {
  ...
  /**
   * This function lets you show whether or not the robot is ready to receive a
   * gear.
   * 
   * @param ready A boolean that indicates whether or not the robot is ready. True
   *              if yes.
   */
  public void showReadyToReceiveGear(final int stateOfReadiness) {
   ...
   */
  public void showReadyToClimb(final boolean ready) {
    if (ready)
  ...
  /**
   * This is used to display a basic pattern on the bling LED lights. Note that
   * for functions above 9, pixelStart and pixelEnd and delay will do nothing.
   * 
   * @param pattern    The type of motion or animation pattern you would like to
   *                   display. Patterns range from 1-12.
   *                   <p>
   *                   1 : Colour wipe
   *                   <p>
   ...
   *                   <p>
   * @param colour     Colour, either as a preset such as "RED", "GREEN", "WHITE"
   *                   (either caps or no caps) or in decimal format. Use a
   *                   programmer calculator to determine decimal format.
   *                   <p>
   * 
   *                   Presets: GREEN, RED, BLUE, YELLOW, ORANGE, PURPLE, TAN,
   *                   VIOLET, MERGE, PINK, WHITE, TURQUOISE, BLACK, GOLD, SILVER
   * @param delay      The delay between animation segments in seconds, if
   *                   applicable.
   * @param brightness The brightness of the LED pattern as an integer between 0
   *                   and 100.
   * @param pixelStart The percent of the bar where the pixel pattern will start
   *                   in decimal format.
   * @param pixelEnd   The percent of the bar where the pattern will end in
   *                   decimal format.
   */
  public void customDisplay(final String colour, int pattern, final double tDelay, final int brightness,
      final int pixelStart, final int pixelEnd) {

    final int delay = (int) Math.round(tDelay);
    ...
    final int startPixel = Math.round(pixelStart * pixels);
    final int endPixel = Math.round(pixelEnd * pixels);
