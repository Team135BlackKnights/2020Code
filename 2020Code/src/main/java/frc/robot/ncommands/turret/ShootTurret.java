...
public class ShootTurret extends CommandBase {
  ...
  public void execute() {
    ...
    /*
     * if(turret.isBallInShooter()) { turret.runShooterRPM(_topShooterRPM,
     * _bottomShooterRPM); turret.runBallFeeder(0); isFinished = false; } else
     * if(turret.isBallInShooter() &&turret.turretBallCount == 0) {
     * turret.runBallFeeder(.75); isFinished = false; } else
     * if(turret.isBallInShooter() &&turret.turretBallCount >= 1) {
     * turret.turretBallCount--; isFinished = true; } else
     * if(!turret.isBallInShooter() &&turret.turretBallCount >=1) {
     * turret.runBallFeeder(.75); turret.turretBallCount--; isFinished = false; }
     * else { turret.runShooterRPM(_topShooterRPM, _bottomShooterRPM);
